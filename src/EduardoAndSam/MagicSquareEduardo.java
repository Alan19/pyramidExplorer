...
//		magicSquare[2][2] = 8;
		...
		boolean colHasItem = false;
		for(int row = 0; row < magicSquareUn; row++){
			for(int col; col < magicSquareUn[row]; col++){
				if(magicSquareUn[row][col] != 0){
					rowHasItem = true;
				}
			}
			if(!rowHasItem){
				int colPlace = (int)(4*Math.random()+1);
				magicSquareUn[row][colPlace] = magicSquare[row][colPlace];
			}
		}
